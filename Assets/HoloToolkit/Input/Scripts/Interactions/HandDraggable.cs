...
namespace HoloToolkit.Unity.InputModule {
    ...
    public class HandDraggable : MonoBehaviour,
        ...
        private List<Transform> imageTargetChildren;
        ...
        private void UpdateDragging() {
            ...
            Vector3 mainCameraTransformDirection = mainCamera.transform.TransformDirection(objRefGrabPoint);
            if (first) {
                initRotation = HostTransform.rotation;
            ...
            switch (gameObject.name) {
                case "X":
                    if (draggingPosition.x < HostTransform.position.x) {
                        draggingPosition.x = HostTransform.position.x;
                        mainCameraTransformDirection.x = 0;
                    }
                    draggingPosition.y = y;
                    mainCameraTransformDirection.y = 0;
                    draggingPosition.z = z;
                    mainCameraTransformDirection.z = 0;
                    break;
                case "Y":
                    draggingPosition.x = x;
                    mainCameraTransformDirection.x = 0;
                    if (draggingPosition.y < HostTransform.position.y) {
                        draggingPosition.y = HostTransform.position.y;
                        mainCameraTransformDirection.y = 0;
                    }
                    draggingPosition.z = z;
                    mainCameraTransformDirection.z = 0;
                ...
                case "Depth":
                    ...
                    draggingPosition.y = y;
                    mainCameraTransformDirection.y = 0;
                    if (draggingPosition.z > HostTransform.position.z) {
                        draggingPosition.z = HostTransform.position.z;
                        mainCameraTransformDirection.z = 0;
                    }
                    break;
            }

            HostTransform.position = Vector3.Lerp(HostTransform.position, mainCameraTransformDirection + draggingPosition, PositionLerpSpeed);
            ...
            HostTransform.rotation = initRotation;
